...
- **§9 — one engine effect per run-scoped signal.** `applySignal` (`src/core/signals.ts`) is the single implementation of `step-done`/`ask-human`/`bounce`/`capture-attempt`, called by BOTH the HTTP handler (`server/app.ts`, thin shells) and the CLI's in-process fallback (`cli/index.ts`, via `dispatchSignal`) — so the effect logic and the lock discipline (`withRunLock` for the fire-and-forget `step-done`, `withRunLockWaiting` for the rest, matching each descriptor's `lockDiscipline`) live in exactly one place instead of duplicated across two files with hand-picked locks. `capture-attempt` carries its explicit `step` arg; `evidence-upload` is scoped `product-outbox` + routed through the durable `evidence_uploads` outbox; the `@@*_CMD@@` prompt tokens come from the registry (`signalCommand`). Seam covered by `test/reconcile.test.ts` ("applySignal — shared run-scoped agent signal effect").
...
- **§9 — per-signal route/command *declarations* stay explicit** (each signal's zod body/response schema + a thin route + a thin CLI command owning its arg parsing / output formatting); those thin shells delegate to `applySignal` rather than being *generated* by iterating `SIGNAL_DESCRIPTORS`. The effect fan-out the design railed against is gone; the declaration boilerplate (irreducibly per-signal) is not looped away.
- **§9 — `capture-lock` is not a `SignalDescriptor`** and `exclusive_resource` is a declared-but-unused `GuardKind`; the capture mutex is still a standalone `capture-lock acquire|release` CLI command. `evidence-upload` likewise stays a CLI-local `product-outbox` action, not an HTTP route.
