...
if [[ -n "$HERDR_CATS_REPO" && -f "$HERDR_CATS_CONFIG_DIR/repos/$HERDR_CATS_REPO.conf" ]]; then
  # shellcheck disable=SC1090
  source "$HERDR_CATS_CONFIG_DIR/repos/$HERDR_CATS_REPO.conf"
fi
...
HERDR_CATS_BOOTSTRAP_CMD="${HERDR_CATS_BOOTSTRAP_CMD:-}"   # e.g. "mise run setup"
HERDR_CATS_DESLOP_CMD="${HERDR_CATS_DESLOP_CMD:-}"         # e.g. "/reckon-de-slop-app-code"
HERDR_CATS_RESOLVE_CMD="${HERDR_CATS_RESOLVE_CMD:-}"       # e.g. "/reckon-resolve-pr"
HERDR_CATS_BRIEF_GUIDANCE="${HERDR_CATS_BRIEF_GUIDANCE:-}" # path to repo-specific guidance appended to the brief
