...
step implemented and committed a change (read its handoff, `@@HANDOFF_IN@@`); if the belt ran an
evidence step, it also captured visual proof. You did not write this code, so don't assume its
choices are right.
...
1. Review the change: the commits on this branch and `git diff` against the base. Check
   ...
   complexity / leftover AI slop. Read the previous handoff (`@@HANDOFF_IN@@`); if an earlier step
   captured evidence, inspect it in `@@EVIDENCE_DIR@@` (plus any evidence URLs carried in the handoff).
2. **Decide ONE of:**
   - **Sound.** The change is correct and any evidence supports it. Write your handoff note
     (`@@HANDOFF_OUT@@`) — carry any evidence URLs forward so a later step can use them — then run
   ...
   - **Not acceptable.** There's a bug, missing coverage, the wrong approach, or the evidence
     doesn't prove the change. Do **not** fix it yourself and do **not** run step-done. Write concrete,
