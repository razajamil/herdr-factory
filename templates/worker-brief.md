...
- Do NOT transition the Jira ticket. After opening the PR, attach the evidence
  (step 8) and see it through its immediate automated round (CI green + bot
  comments, step 9), then stop — the dispatcher watches for human review
  comments afterwards.
...
7. **Commit** (code only — `@@EVIDENCE_DIR@@` is gitignored, so the screenshots are
...
8. **Attach the screenshots inline (best-effort):** if you captured any, add them
...
9. **Wait for automated feedback (up to ~10 minutes).** Do NOT stop right after
...
10. **Signal done + stop.** Run `@@CATS_CLI@@ --repo @@REPO@@ worker-done @@KEY@@`
