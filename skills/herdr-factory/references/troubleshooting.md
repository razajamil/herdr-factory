...
| …and NO `layout_applied` event exists at all, on a belt whose other steps have no `tab`/`pane` | the layout was never built: the first dedicated-pane spawn added a tab, and the hook only builds into a FRESH 1-tab/1-pane worktree | Give **every** step of that belt a `tab`/`pane` from the layout (see [layouts.md](./layouts.md)) |
